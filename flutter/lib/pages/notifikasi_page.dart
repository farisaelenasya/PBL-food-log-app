...
// ─── Service: Generate notif ─────────────────────────────────────────────────

class NotifService {
  static const double batasRendah = 70;
  static const double batasTinggi = 180;
    ...
    for (int i = 0; i < entri.length; i++) {
      ...
      if (e.nilai < batasRendah) {
        hasil.add(ItemNotif(
          ...
          isi: 'Kadar gula Anda ${e.nilai.toStringAsFixed(0)} mg/dL saat ${e.konteksMakan}. '
  ...
  static List<ItemNotif> notifInfo() => [
        ItemNotif(
          ...
          tipe: TipeNotif.info,
          judul: '💡 Tips Hari Ini',
          isi: 'Jalan kaki 15 menit setelah makan dapat membantu menstabilkan gula darah Anda.',
...
class _NotifikasiPageState extends State<NotifikasiPage> {
  ...
  Future<void> _muatNotif() async {
    ...
    try {
      final obatList = await _medService.getMedications();

      final gulaNotif = NotifService.buatNotifGula(_glucoseStore.semuaEntri);
      final obatNotif = NotifService.buatNotifObat(obatList);
      final infoNotif = NotifService.notifInfo();

      final semua = [...gulaNotif, ...obatNotif, ...infoNotif];
      semua.sort((a, b) => b.waktu.compareTo(a.waktu));
      ...
      setState(() {
        _errorPesan = e.toString().replaceFirst('Exception: ', '');
        _sedangMemuat = false;
      });
    }
  }
  ...
  Widget _buildRingkasanAlert() {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Text('🔔 Ringkasan Alert Gula Darah',
              style: TextStyle(
                  color: Colors.white,
                  fontWeight: FontWeight.bold,
                  fontSize: 13)),
          ...
          Row(
            children: [
              Expanded(
                  child: _chipRingkasan('${jumlahRendah}x Rendah',
                      Icons.arrow_downward_rounded, const Color(0xFFFFB300))),
              const SizedBox(width: 10),
              Expanded(
                  child: _chipRingkasan('${jumlahTinggi}x Tinggi',
                      Icons.arrow_upward_rounded, const Color(0xFFEF5350))),
            ],
          ),
          const SizedBox(height: 10),
          const Text('Target normal: 70 – 180 mg/dL',
  ...
  Widget _buildKartu(ItemNotif notif) {
    ...
    return Dismissible(
      ...
      background: Container(
        ...
        decoration: BoxDecoration(
            color: Colors.red[400], borderRadius: BorderRadius.circular(18)),
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            border: notif.sudahDibaca
                ? null
                : Border.all(
                    color: style.iconColor.withValues(alpha: 0.25), width: 1.2),
            boxShadow: [
              BoxShadow(
